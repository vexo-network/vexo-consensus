...
- RPC admin policy, TLS identity, and node configuration
...
- Operators configure TLS identity, optional RPC root or scoped admin tokens, optional P2P replay-resistant auth proofs, request limits, peer scoring, `MaxScore`, and ban thresholds. Admin RPC endpoints must stay behind an operator network, mTLS boundary, or authenticated gateway when bearer tokens are not configured, and embeddings should attach the RPC admin audit sink to their structured log pipeline.
...
- VRF-backed committee selection requires an adapter with proof verification, dependency-audit metadata, pinned audit-evidence SHA-256, and key-source evidence; the built-in ECVRF P-256 adapter provides local encrypted-key custody and the `remote-vrf-http-v1` adapter provides the KMS/HSM remote-prover boundary. `vexod keys serve-vrf` persists replay nonces and audit events by default, and embedded services can require a durable `RemoteVRFReplayStore`. Private key custody, remote service availability, access policy, durable storage placement, and deployment audit remain operator responsibilities.
