...
- Data availability commitments use canonical transaction chunk roots with chunk-inclusion proofs and one-missing-chunk-per-group XOR parity recovery. Stronger many-missing-chunk recovery or probabilistic 2D sampling should be added by chains that require Reed-Solomon-grade DA.
- Invalid-proposal evidence supports reason-specific mismatch verification. Transaction-validity evidence is gated by an independently computed deterministic transaction-result hash; without that context it fails closed. This is still not a complete light-client Merkle/state-proof framework for every possible invalid proposal claim.
