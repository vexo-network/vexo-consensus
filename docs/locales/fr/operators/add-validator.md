...
Ce document couvre l’ajout d’un validator, la validation de configuration et les contrôles de staking. Les commandes, champs JSON, noms RPC, config key et identifiants de code utilisés par l’implémentation et l’exploitation restent en anglais pour préserver la compatibilité.
