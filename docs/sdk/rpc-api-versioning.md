...
- `modules/evm/ethcompat.RunTransactionFixturesJSON`, `modules/evm/backend/geth.RunExecutionFixturesJSON`, `vexod ops conformance --evm-tx-fixtures <file>`, and `vexod ops conformance --evm-execution-fixtures <file>` are the CI entry points for Ethereum raw-transaction and VM execution fixture/conformance suites. The built-in `--evm-default-fixtures` corpus covers dynamic-fee calls, contract creation, access-list transactions, protected legacy transactions, wrong-chain rejection, malformed raw rejection, base-fee rejection, priority-fee rejection, unprotected legacy rejection, geth call return data, contract creation execution, revert behavior, and persistent storage writes. Keep larger fixture corpora outside normal source unless they are small, licensed, and intentionally versioned.
