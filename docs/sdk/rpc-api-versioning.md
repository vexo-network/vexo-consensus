...
- `modules/evm/ethcompat.RunTransactionFixturesJSON` and `vexod ops conformance --evm-tx-fixtures <file>` are the CI entry points for Ethereum raw-transaction fixture/conformance suites. The built-in `--evm-default-fixtures` corpus covers dynamic-fee calls, contract creation, access-list transactions, protected legacy transactions, wrong-chain rejection, malformed raw rejection, base-fee rejection, priority-fee rejection, and unprotected legacy rejection. Keep larger fixture corpora outside normal source unless they are small, licensed, and intentionally versioned.
