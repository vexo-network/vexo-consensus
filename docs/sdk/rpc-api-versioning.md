...
- Operators can call `/v1/capabilities` to see which RPC provider interfaces are actually attached. When `require_network_safety` is enabled, `vexod start` requires the full built-in node RPC capability set before the server is allowed to listen. SDK embedders should prefer `rpc.NewNetworkSafeServer` or `rpc.NewNetworkSafeHandlerWithConfig`; lower-level integrations can still set `rpc.Config.RequiredCapabilities` or `rpc.Config.RequireAllCapabilities` so a partial provider fails at startup instead of returning `501` later.
...
- WebSocket `newHeads`, `logs`, and `newPendingTransactions` delivery is bounded per polling tick, per connection, and by idle deadline. Socket writes use deadlines so slow clients cannot force unbounded catch-up work. Embedders can tune `rpc.Config.Web3SubscriptionInterval`, `Web3SubscriptionMaxCatchUp`, `Web3SubscriptionMaxLogBatch`, `Web3SubscriptionMaxPendingRun`, `Web3SubscriptionMaxPerConn`, and `Web3SubscriptionIdleTimeout`; split config files expose the last two as `network_config.json` RPC fields `web3_max_subscriptions_per_connection` and `web3_idle_timeout`.

### Operator Latency Metrics

`/v1/metrics` exposes average plus p95/p99 latency for proposal, vote, and commit processing. Alert on sustained p95 or p99 growth before average latency moves; this catches slow peers, saturated disks, and overloaded RPC/Web3 subscription fanout earlier than height-only monitoring.
...
- Native EVM module call queries should encode transfer value as `value_wei` decimal or `value_hex` hex for full unsigned 256-bit semantics. The older numeric `value` field is retained only for small legacy uint64 callers, and conflicting `value_wei`/`value_hex` encodings are rejected.
...
- Optional managed Web3 accounts require `network_config.json` RPC `evm_managed_accounts: true` plus either `evm_account_key_envs` entries or `evm_account_private_keys` entries. Repeated `vexod start --evm-account-key-env` is available for private operator workflows; repeated `--evm-account-key` remains supported for local-only testing but is unsafe because command-line arguments can leak through process inspection and shell history. Managed accounts power `eth_accounts`, `eth_coinbase`, `eth_sign`, `personal_sign`, `eth_signTransaction`, and `eth_sendTransaction`; `eth_sendTransaction` signs an Ethereum-format transaction locally, verifies it with the configured Vexo EVM chain ID, then submits the translated Vexo canonical transaction. Startup safety rejects managed EVM hot keys on public RPC listeners. Production operators should prefer external wallets, env/secret-manager injection, or remote signing flows instead of plain JSON private keys.
...
- `modules/evm/ethcompat.RunTransactionFixturesJSON`, `modules/evm/backend/geth.RunExecutionFixturesJSON`, `vexod ops conformance --evm-tx-fixtures <file>`, and `vexod ops conformance --evm-execution-fixtures <file>` are the CI entry points for Ethereum raw-transaction and VM execution fixture/conformance suites. The built-in `--evm-default-fixtures` corpus covers dynamic-fee calls, contract creation, CREATE2, access-list transactions, protected legacy transactions, wrong-chain rejection, malformed raw rejection, base-fee rejection, priority-fee rejection, unprotected legacy rejection, geth call return data, contract creation execution, revert behavior, persistent storage writes, event logs, value transfer, precompile execution, access-list gas semantics, and blob-hash semantics. Use `--evm-tx-fixtures-sha256` and `--evm-execution-fixtures-sha256` to pin external fixture files or deterministic directory corpora in CI; release-gate evidence rejects built-in, dry-run, placeholder, or zero-fixture corpus sources. Keep larger fixture corpora outside normal source unless they are small, licensed, and intentionally versioned.
