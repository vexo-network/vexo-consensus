...
Contract VM adapters return `contract.Result`. The default `evm` adapter is backed by go-ethereum's EVM interpreter and executes EVM bytecode for deploy/call paths. Geth-specific code is isolated under `modules/evm/backend/geth`, so geth API changes should be contained to that backend package and its conformance tests. The EVM module persists runtime code, `StorageWrites` to `evm/storage/{address}/{slot}`, receipts by transaction hash, and logs by height/transaction/log index plus address. This keeps `eth_getCode`, `eth_getStorageAt`, `eth_getTransactionReceipt`, address-scoped `eth_getLogs`, and global `eth_getLogs` backed by committed module state instead of process memory.
