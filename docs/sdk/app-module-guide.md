...
Contract VM adapters return `contract.Result`. The default `evm` adapter is backed by go-ethereum's EVM interpreter and executes EVM bytecode for deploy/call paths. Geth-specific interpreter code is isolated under `modules/evm/backend/geth`, and signed Ethereum transaction decoding plus Ethereum transaction/receipt trie root calculation are isolated under `modules/evm/ethcompat`, so geth API changes should be contained to those compatibility packages and their conformance tests. `eth_sendRawTransaction` accepts signed Ethereum legacy/access-list/dynamic-fee typed transactions, verifies sender recovery and chain ID, preserves the Ethereum transaction hash, and maps calls or contract creations into Vexo canonical `evm` transactions. Ethereum contract creation uses the standard sender/nonce CREATE address. The EVM module persists runtime code, `StorageWrites` to `evm/storage/{address}/{slot}`, VM balance writes into the shared `bank` namespace, receipts by transaction hash, and logs by height/transaction/log index plus address. Ethereum hex account keys are normalized before bank reads/writes, so checksum and lowercase forms of the same 20-byte address resolve to one account. This keeps `eth_getBalance`, `eth_getCode`, `eth_getStorageAt`, `eth_getTransactionReceipt`, `eth_getBlockReceipts`, address-scoped `eth_getLogs`, and global `eth_getLogs` backed by committed module state instead of process memory.
