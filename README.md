...
Available endpoints: `/healthz`, `/readyz`, `/status`, `/metrics`, `/peers`, `/tx`, `/evidence`, `/prune`, `/replay`, `/blocks`, `/blocks/latest`, `/blocks/{height}`, `/state/latest`, `/state/{height}/{namespace}`, `/validators/{height}`, `/committee/{height}/{round}`.
