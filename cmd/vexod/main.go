...
func writeHelp(writer io.Writer) {
	...
	fmt.Fprintf(writer, "  upgrade update  generate a version-pinned upgrade plan from only a version label\n")
	fmt.Fprintf(writer, "\nCommon Subcommands:\n")
	...
	fmt.Fprintf(writer, "  relayer soak-plan build an IBC relayer soak-test config\n")
	fmt.Fprintf(writer, "  network analyze-longrun analyze long-run evidence JSON\n")
	fmt.Fprintf(writer, "  ops conformance validate SDK, ops, and EVM fixture conformance\n")
	fmt.Fprintf(writer, "  release collect-evidence collect RPC-backed release evidence artifacts\n")
	fmt.Fprintf(writer, "  release docs-quality validate canonical and localized documentation quality\n")
